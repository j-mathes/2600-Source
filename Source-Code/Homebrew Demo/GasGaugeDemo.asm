[stella] Gas gauge demo<-prev [Thread] next-><-prev [Date] next-> 
      Month Index | List Home 

[stella] Gas gauge demo
      Subject: [stella] Gas gauge demo
      From: Joe Grand <jgrand@xxxxxxxxxxxxxx>
      Date: Fri, 23 Mar 2001 01:47:27 -0500


Alright, I wasn't expecting to be up until almost 2am working on this, but oh 
well :)

Attached is a source and binary for the horizontal "gas gauge" demo. I am 
planning on turning this into the Data Latency Buffer for my game..

I ended up doing exactly what Manuel suggested:


<-- Red --><--------- normal BG color ------------>
               |
          Cycle X
The background is drawn in the above fashion, starting red at the beginning of 
the scanline and changing to black X cycles later. The playfield graphics are 
used for the actual gauge, which when full will cover over all the red (and a 
bit more, which looks sloppy to me, but we can hopefully fix that later - check 
the binary to see what I mean).

PF graphics obviously don't give the best resolution, but I'm using PF0 and PF1 
for a 12-pixel wide gauge, which should be good enough.

For this demo, moving the joystick up or down will increase or decrease the 
gauge.

As a side note, I would have liked to been able to horizontally position the 
gauge more towards the right of the screen, but my timing would get all wacky 
and I couldn't seem to change back to the normal BG color in time. If anyone 
wants to attempt it, feel free :)
...
;Table6: .byte   $00, $00, $00, $00, $00, $00, $00, $00  ;PF2 right half
        ...
        .word      Start   ; reset
        .word      Start   ; IRQ
Attachment: gauge.bin
Description: Binary data
      Current Thread
        [stella] Gas gauge demo 
          Joe Grand - Fri, 23 Mar 2001 01:43:55 -0500 (EST) <= 
            Manuel Polik - Sat, 24 Mar 2001 03:58:55 -0500 (EST) 


      <- PreviousIndexNext ->
      [stella] Dave Neuman/Brownstown/Bra, DNeuman ThreadRe: [stella] Gas gauge 
      demo, Manuel Polik 
      Re: [stella] Gas gauge - revisited, Joe Grand Date[stella] Qb, 2600, PC, 
      adios, Andrew Davie 
      Month

      List hosting by BIGLIST 
